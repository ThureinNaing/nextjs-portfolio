...
const SparkleNavbar: React.FC<SparkleNavbarProps> = ({
	...
	const animateToTab = useCallback(
		(index: number, currentActive: number) => {
			if (index === currentActive || isAnimating.current) return;

			const navElement = navRef.current;
			const activeElement = activeElementRef.current;
			const oldButton = buttonRefs.current[currentActive];
			...
			const direction = index > currentActive ? "after" : "before";
			...
			entries.forEach((entry) => {
				if (entry.isIntersecting && !isAnimating.current) {
					...
					if (index !== -1 && index !== activeIndex) {
						animateToTab(index, activeIndex);
					}
				}
			});
		}, observerOptions);
	...
	}, [items, activeIndex, animateToTab]); // Fixed warning 1
	...
	}, [activeIndex, getOffsetLeft]); // Fixed warning 2

	return (
		<>
			<style>{`
        /* ORIGINAL STYLES RESTORED */
        .navigation-menu { margin: 20px 0px; position: relative; z-index: 1; }
        .navigation-menu ul { margin: 0; padding: 0; list-style: none; display: flex; gap: 40px; }
        .navigation-menu ul li button { -webkit-appearance: none; border: none; cursor: pointer; background-color: transparent; padding: 0; margin: 0; transition: color 0.25s; }
        ...
        .navigation-menu .active-element {
          ...
          opacity: var(--active-element-show); pointer-events: none;
        }
        ...
        .navigation-menu .active-element > svg { display: block; overflow: visible; height: 5px; filter: blur(0.5px) drop-shadow(0px 0px 8px ${color}); }
        .navigation-menu .active-element .strike {
          padding: 24px 0;
          mask-image: linear-gradient(to right, transparent var(--active-element-mask-position), black calc(15% + var(--active-element-mask-position)), black 80%, transparent);
          -webkit-mask-image: linear-gradient(to right, transparent var(--active-element-mask-position), black calc(15% + var(--active-element-mask-position)), black 80%, transparent);
        }
        .navigation-menu .active-element .strike svg {
          display: block; overflow: visible; height: 12px; width: calc(var(--active-element-width) * 2);
          transform: translate(var(--active-element-strike-x), 30%) scale(var(--active-element-scale-x), var(--active-element-scale-y));
        }
        .navigation-menu .active-element .strike svg:last-child { transform: translate(var(--active-element-strike-x), -30%) scale(-1); }
						...
						>
							...
							>
								<button
									...
									className="text-foreground italic font-bold text-sm"
								>
									{item}
								</button>
							</a>
